...
### 一、Schema 演进与元数据管理
- [1.1 默认值机制](#11-默认值机制) 🟢
- [1.2 零成本列重命名](#12-零成本列重命名) 🟢
- [1.3 Parquet 文件的 ID 映射](#13-parquet-文件的-id-映射) 🟡
- [1.4 fieldId 写入机制](#14-fieldid-写入机制) 🟡
- [1.5 Name Mapping 机制](#15-name-mapping-机制) 🔴
- [1.6 默认值类型详解](#16-默认值类型详解) 🟢
- [1.7 ConstantReader 实现](#17-constantreader-实现) 🟡
- [1.8 默认值职责分离](#18-默认值职责分离) 🟠
- [1.9 Hive 迁移与默认值](#19-hive-迁移与默认值) 🔴

### 二、事务与并发控制
- [2.1 ACID 特性实现](#21-acid-特性实现) 🟠
- [2.2 并发写入冲突处理](#22-并发写入冲突处理) 🟡
- [2.3 Flink 幂等性保证](#23-flink-幂等性保证) 🔴

### 三、Flink 流式集成
- [3.1 Exactly-Once 语义](#31-exactly-once-语义) 🟠
- [3.2 两阶段提交机制](#32-两阶段提交机制) 🟠
- [3.3 prepareSnapshotPreBarrier 作用](#33-preparesnapshotprebarrier-作用) 🟡
- [3.4 Committer 单并行度设计](#34-committer-单并行度设计) 🟠
- [3.5 Position Delete vs Equality Delete](#35-position-delete-vs-equality-delete) 🟡
- [3.6 Merge-on-Read 优化](#36-merge-on-read-优化) 🔴
- [3.7 Upsert 模式优化](#37-upsert-模式优化) 🟡
- [3.8 删除文件的产生机制](#38-删除文件的产生机制) 🟠
- [3.9 删除文件的小文件优化](#39-删除文件的小文件优化) 🟠
- [3.10 Tag 与快照的关系](#310-tag-与快照的关系) 🟡
- [3.11 小文件合并后的历史文件删除](#311-小文件合并后的历史文件删除) 🟠

### 四、文件管理与优化
- [4.1 小文件合并策略](#41-小文件合并策略) 🟠
- [4.2 垃圾回收机制](#42-垃圾回收机制) 🟡
- [4.3 Manifest 优化](#43-manifest-优化) 🟠
- [4.4 孤儿文件清理安全机制](#44-孤儿文件清理安全机制) 🟡

### 五、查询与读取优化
- [5.1 多层剪枝机制](#51-多层剪枝机制) 🟡
- [5.2 谓词下推优化](#52-谓词下推优化) 🟡
- [5.3 时间旅行实现](#53-时间旅行实现) 🟢
- [5.4 Spark vs Flink 读取对比](#54-spark-vs-flink-读取对比) 🟠

### 六、Spark 与 Flink 读取机制深度解析
- [6.1 FileScanTask 结构与设计](#61-filescan task-结构与设计) 🟡
- [6.2 DeleteFile 关联机制](#62-deletefile-关联机制) 🟠
- [6.3 Merge-on-Read 实现原理](#63-merge-on-read-实现原理) 🟠
- [6.4 Position Delete 应用机制](#64-position-delete-应用机制) 🟡
- [6.5 Equality Delete 应用机制](#65-equality-delete-应用机制) 🟡
- [6.6 Schema 投影与 Delete 协同](#66-schema-投影与-delete-协同) 🟠
- [6.7 Sequence Number 并发控制](#67-sequence-number-并发控制) 🔴
- [6.8 Flink Incremental Scan 实现](#68-flink-incremental-scan-实现) 🟠
- [6.9 Spark DeleteFilter 优化](#69-spark-deletefilter-优化) 🟡

### 七、Catalog 与元数据管理架构
- [7.1 Catalog 抽象架构与 SPI 机制](#71-catalog-抽象架构与-spi-机制) 🟠
- [7.2 TableOperations 的 commit 原子性保证](#72-tableoperations-的-commit-原子性保证) 🔴
- [7.3 REST Catalog 与传统 Catalog 的对比](#73-rest-catalog-与传统-catalog-的对比) 🟡

### 八、SnapshotProducer 提交核心流程
- [8.1 SnapshotProducer 模板方法模式](#81-snapshotproducer-模板方法模式) 🔴
- [8.2 Manifest 自动合并与分裂机制](#82-manifest-自动合并与分裂机制) 🟠
- [8.3 Snapshot Summary 累加器设计](#83-snapshot-summary-累加器设计) 🟡

### 九、DeleteFileIndex 与删除文件匹配优化
- [9.1 DeleteFileIndex 多维索引结构](#91-deletefileindex-多维索引结构) 🔴
- [9.2 Equality Delete 的 Range Overlap 剪枝](#92-equality-delete-的-range-overlap-剪枝) 🟠
- [9.3 Deletion Vector 与 Roaring Bitmap 实现](#93-deletion-vector-与-roaring-bitmap-实现) 🟡

### 十、面试必杀技 - 能让你脱颖而出的源码细节
- [10.1 Flink CommitDeltaTxn 为何不合并事务](#101-flink-commitdeltatxn-为何不合并事务) 🔴
- [10.2 IcebergFilesCommitter 的 NavigableMap 设计](#102-icebergfilescommitter-的-navigablemap-设计) 🟠
- [10.3 Iceberg 为何选择 Roaring Bitmap](#103-iceberg-为何选择-roaring-bitmap) 🟡
- [10.4 表格式版本演进 V1→V2→V3 核心变化](#104-表格式版本演进-v1v2v3-核心变化) 🟠
...
**🟠 问题: Spark 在构建 FileScanTask 时,如何判断哪些 DeleteFile 需要关联到特定的 DataFile？**

**答案:**

Iceberg 根据 DeleteFile 类型采用不同的关联策略。

#### DeleteFile 接口定义

```java
// 源码: api/src/main/java/org/apache/iceberg/DeleteFile.java
public interface DeleteFile extends ContentFile<DeleteFile> {
    /**
     * 返回此删除文件引用的数据文件路径
     * - Position Delete 可能返回具体路径(单文件删除)
     * - 返回 null 表示可能影响多个 DataFile
     * - Equality Delete 总是返回 null
     */
...
| DeleteFile 类型      | 关联方式                              | 关联范围          |
|---------------------|---------------------------------------|-------------------|
| **Position Delete** | 匹配 `file_path` 列或 `referencedDataFile()` | 特定 DataFile     |
| **Equality Delete** | 无需匹配,全局关联                    | 所有 DataFile     |

#### Position Delete 关联逻辑
...
**场景2: 全局 Position Delete**

当 `referencedDataFile() == null` 时,需要读取 DeleteFile 的 `file_path` 列来判断:
...
#### Equality Delete 关联逻辑

Equality Delete **总是全局关联**,影响所有 DataFile:

```java
// ContentFile 接口中的关键方法
...
**全局生效原因**:
- 等值删除不依赖文件路径,依赖数据值
- 任何 DataFile 中 `id=1001 AND email='alice@example.com'` 的行都会被删除
...
|------------------|----------------------|-----------------------|
| 删除精度         | 精确到行位置         | 基于字段值匹配        |
| 影响范围         | 单个或多个 DataFile   | 全表所有 DataFile      |
| 存储开销         | 小(仅路径+位置)    | 中等(需存储等值字段)|
| 读取性能         | 高(直接跳过行)     | 中(需 Join 匹配)      |
| 使用场景         | UPDATE/DELETE 单行    | 批量 DELETE 条件删除    |
...
**🔴 问题: dataSequenceNumber 和 fileSequenceNumber 有什么区别？它们如何确保 Delete 的正确性？**

**答案:**

Iceberg 通过双序列号机制优雅解决了 Compaction 后 Delete 生效性问题。
...
| **Compaction 场景**   | 保持不变                | 设为新的序列号           |
| **用途**             | 判断 Delete 是否生效      | 文件生命周期追踪         |
...
Snapshot 2 (seq=15):
  - DeleteFile D (dataSeq=10, fileSeq=15)  ← 删除 Snapshot 1 的数据

Snapshot 3 (seq=20) - Compaction:
  - DataFile B (dataSeq=10, fileSeq=20)    ← 压缩 DataFile A
  - DeleteFile D 仍然有效,因为 B 的 dataSeq=10
```

#### 关键规则

> DeleteFile 的 dataSequenceNumber 决定它能删除哪些数据:
> **仅删除 `dataSeq <= deleteFileDataSeq` 的 DataFile 中的行**

这确保了:
- 新写入的数据不会被旧的 Delete 文件误删
- Compaction 后 Delete 仍然正确生效

**深入理解**: 通过 dataSeq 与 fileSeq 双序列号,Iceberg 优雅解决了 Compaction 后 Delete 生效性问题,这是 Copy-on-Write 格式难以实现的。

#### 💡 面试加分话术

> "双序列号机制是 Iceberg 实现**时间旅行安全性**的关键设计:
> 1. **dataSequenceNumber 记录数据诞生时间**: 无论文件如何被压缩重写，数据的逻辑时间戳不变
> 2. **fileSequenceNumber 记录文件物理写入时间**: 用于追踪文件生命周期
> 3. **Delete 生效规则**: `deleteFile.dataSeq >= dataFile.dataSeq` 时才生效
>
> 这解决了一个经典难题: **Compaction 重写文件后，历史 Delete 是否还应该生效？**
> 答案是**必须生效**，因为 Delete 针对的是逻辑数据，而非物理文件。dataSequenceNumber 正是实现这一语义的关键。"

#### 🔥 高频追问

**Q: Equality Delete 的 applySequenceNumber 为什么是 dataSequenceNumber - 1？**

**A**: 这是为了处理**同一事务内**的 Insert 和 Delete:
```
事务 T (seq=10):
  - INSERT row (pk=1, value='A')  → dataSeq=10
  - DELETE row WHERE pk=1         → dataSeq=10, applySeq=9

如果 applySeq=10, 则 DELETE 会删除同一事务的 INSERT
通过 applySeq=9, DELETE 只删除 seq<=9 的数据,保护同事务写入
```
这是 Iceberg 实现**语句级可见性**的精妙设计。

---

### 6.8 Flink Incremental Scan 实现

**🟠 问题: Flink 流式读取 Iceberg 时,如何实现增量扫描？请说明 IncrementalAppendScan 的核心逻辑。**

**答案:**

Flink 通过 `IncrementalAppendScan` 实现持续监控新 Snapshot 并增量读取数据。

#### IncrementalAppendScan 接口
...
**关键特性**:
1. **Exclusive 起点**: `fromSnapshotExclusive` 表示从该 Snapshot 之后开始(不包含)
2. **Inclusive 终点**: `toSnapshot` 表示读到该 Snapshot 为止(包含)
3. **仅读取 APPEND 操作**: 忽略 DELETE/OVERWRITE 等操作的 Snapshot

#### appendsBetween 实现

```java
// 源码: core/src/main/java/org/apache/iceberg/BaseIncrementalAppendScan.java
private static List<Snapshot> appendsBetween(
    Table table, Long fromSnapshotIdExclusive, long toSnapshotIdInclusive) {

    List<Snapshot> snapshots = Lists.newArrayList();

    // 遍历从 toSnapshotId 到 fromSnapshotId 之间的所有 Snapshot
    for (Snapshot snapshot :
            SnapshotUtil.ancestorsBetween(
                toSnapshotIdInclusive, fromSnapshotIdExclusive, table::snapshot)) {

        // 仅保留 APPEND 操作的 Snapshot
...
- **DELETE**: 仅产生 DeleteFile,没有新 DataFile
...
Snapshot 100 (APPEND)    ← 初始状态
    ↓
Snapshot 101 (APPEND)    ← 新增 100 行
    ↓
Snapshot 102 (DELETE)    ← 删除 10 行(仅 DeleteFile)
    ↓
Snapshot 103 (APPEND)    ← 新增 50 行
    ↓
Snapshot 104 (OVERWRITE) ← 覆盖分区(危险!)

Incremental Scan (100, 104]:
  ✅ 包含: Snapshot 101 (100 行新数据)
  ✗  忽略: Snapshot 102 (DELETE 操作)
  ✅ 包含: Snapshot 103 (50 行新数据)
  ✗  忽略: Snapshot 104 (OVERWRITE 不安全)
...
**🟡 问题: Spark 读取 Iceberg 时有哪些针对 DeleteFilter 的优化？**

**答案:**

Spark 提供了 Executor 缓存和 Runtime Filter 等优化机制。

#### Executor 缓存机制

```java
// 源码: spark/v3.5/spark/src/main/java/org/apache/iceberg/spark/source/BaseReader.java
@Override
...
**CachingDeleteLoader 特性**:
1. **Executor 级别缓存**: 同一 Executor 上多个 Task 共享 DeleteFile
2. **LRU 淘汰策略**: 缓存空间有限时淘汰最久未使用的
3. **适用场景**:
   - 大量小 DeleteFile
   - 多个 DataFile 引用相同 DeleteFile
   - Executor 内存充足

**配置参数**:
```scala
spark.read
  .option("cache-delete-files-on-executors", "true")
  .table("my_table")
```

**示例场景**:
```
Executor 上有 3 个 Task:
...
无缓存: 读取 3 次 delete-001.parquet
有缓存: 读取 1 次 delete-001.parquet,节省 2 次 IO
```

#### Runtime Filter 动态过滤

Spark 在执行 Join 时动态生成过滤条件:
...
3. 将此 Filter 推送到 large_table 的 Scan
4. SparkBatchQueryScan.filter() 过滤掉不匹配的 FileScanTask
```

**优势**:
- 减少需要读取的 DataFile 数量
- 在 Scan 阶段就过滤,避免读取无用数据
- 对于分区表效果显著
