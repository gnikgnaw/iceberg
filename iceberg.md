...
- 主链路：先定位 `parquet/src/main/java/org/apache/iceberg/parquet/ParquetSchemaUtil.java`（`hasIds()` 检测 + `addFallbackIds()` 回退），再联动 `parquet/src/main/java/org/apache/iceberg/parquet/TypeToMessageType.java`（写入时注入 ID），把"检测-回退-注入"串成一条线。
- 面试表达：先回答"是什么"，再补"为什么这样设计"，最后给"边界条件/取舍"。
