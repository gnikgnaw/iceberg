...
#### SparkV2Filters / SparkFilters：类型映射

**文件位置：** `spark/v3.5/spark/src/main/java/org/apache/iceberg/spark/SparkV2Filters.java`（V2 Predicate API，`SparkScanBuilder.pushPredicates()` 实际调用此类），以及 `spark/v3.5/spark/src/main/java/org/apache/iceberg/spark/SparkFilters.java`（旧版 V1 Filter API）。

> **注意**：Spark DataSource V2 中 `SparkScanBuilder.pushPredicates()` 使用的是 `SparkV2Filters.convert(predicate)` 而非旧版的 `SparkFilters`。两者的映射逻辑类似，但适配的 Spark API 版本不同。
...
用户 SQL WHERE 子句
    │
    ├── SparkV2Filters.convert() / FlinkFilters.convert()
    │       将引擎 Filter/Predicate 转为 Iceberg Expression
    ▼
...
| `spark/v3.5/.../SparkV2Filters.java` | Spark V2 Predicate → Iceberg Expression |
| `spark/v3.5/.../SparkFilters.java` | Spark V1 Filter → Iceberg Expression（旧版） |
