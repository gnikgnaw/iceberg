...
**源码位置**: `api/src/main/java/org/apache/iceberg/ManifestFile.java`（第 68-83 行）
...
`PartitionFieldSummary` 接口（第 222-254 行）提供了四个关键方法：
...
核心逻辑在 `PartitionFieldStats.update()` 方法中（第 84-100 行）：
...
**源码位置**: `core/src/main/java/org/apache/iceberg/ManifestReader.java`（第 237-263 行）
...
`Evaluator` 直接对 `StructLike` 形式的分区值进行表达式求值。在 `ManifestReader.evaluator()` 方法中（第 347-353 行）：
...
| `lower_bounds` | 125 | `Map<Integer, ByteBuffer>` | 列 ID -> 列最小值（序列化为 ByteBuffer） |
| `upper_bounds` | 128 | `Map<Integer, ByteBuffer>` | 列 ID -> 列最大值（序列化为 ByteBuffer） |
...
private boolean eval(ContentFile<?> file) {
  ...
  // Avro 兼容性检查：旧版 Avro 文件可能返回负数 recordCount（第 95-99 行）
  if (file.recordCount() < 0) {
    return ROWS_MIGHT_MATCH;
  }
...
对于涉及 Transform 的表达式（如 `bucket(id, 16) = 5`），`InclusiveMetricsEvaluator` 也能处理。它通过 `BoundTransform` 判断 Transform 是否保序（order preserving），如果保序则可以利用原始列的统计信息（第 579-596 行）：
...
**源码位置**: `parquet/src/main/java/org/apache/iceberg/parquet/ReadConf.java`（第 94-116 行）
...
删除文件使用序列号（Sequence Number）来确定其适用范围。`PositionDeletes` 类（第 670-703 行）中使用有序数组和二分查找实现高效过滤：
...
核心原则：**对于 Position Delete，只有序列号 >= 数据文件序列号的删除文件才会被应用；对于 Equality Delete，使用 `applySequenceNumber = dataSequenceNumber - 1`（第 841 行），即序列号 >= (数据文件序列号 - 1) 的等值删除文件才会被应用**。这种差异确保了与数据文件在同一提交中写入的等值删除也能正确生效。这是 Iceberg MVCC（Multi-Version Concurrency Control）的基础。

### 7.5 等值删除的范围检查优化

对于等值删除，`DeleteFileIndex` 还会通过列统计信息做进一步的范围检查（第 219-280 行）：
...
在 `DeleteFileIndex.Builder` 中（第 576-628 行），加载删除 Manifest 时也会应用 Manifest 级过滤：
...
**验证方法**: 对照 Apache Iceberg 源码逐一验证文档中的类名、方法名、字段名和行号引用

**修正内容**:

1. **ManifestFile.java 行号修正**
   - PARTITION_SUMMARY_TYPE 定义：第 68-89 行 → 第 68-83 行
   - PartitionFieldSummary 接口：第 222-255 行 → 第 222-254 行

2. **InclusiveMetricsEvaluator.java 行号修正**
   - eval() 方法中 Avro 兼容性检查注释：第 95-100 行 → 第 95-99 行
   - eval() 方法整体：第 90-109 行 → 第 90-108 行
   - eq() 方法：第 298-324 行 → 第 298-323 行
   - in() 方法：第 342-386 行 → 第 342-385 行
   - mayContainNull() 方法：第 490-492 行 → 第 490-491 行
   - containsNullsOnly() 方法：第 494-500 行 → 第 494-499 行
   - containsNaNsOnly() 方法：第 502-507 行 → 第 502-506 行
   - transformLowerBound() 方法：第 579-597 行 → 第 579-596 行

3. **PartitionSummary.java 行号修正**
   - PartitionFieldStats.update() 方法：第 84-99 行 → 第 84-100 行

4. **ManifestReader.java 行号修正**
   - entries() 方法：第 237-264 行 → 第 237-263 行
   - evaluator() 方法：第 347-354 行 → 第 347-353 行

5. **DeleteFileIndex.java 行号修正**
   - 核心数据结构：第 70-81 行 → 第 73-80 行
   - forDataFile() 方法：第 151-168 行 → 第 151-167 行
   - canContainEqDeletesForFile() 方法：第 219-281 行 → 第 219-280 行
   - deleteManifestReaders() 方法：第 576-629 行 → 第 576-628 行
   - PositionDeletes 类：第 670-742 行 → 第 670-703 行
   - EqualityDeleteFile.applySequenceNumber 字段初始化：补充行号引用（第 841 行）

6. **ReadConf.java 行号修正**
   - Bloom Filter 三重过滤逻辑：第 96-117 行 → 第 94-116 行

7. **ZOrderByteUtils.java 行号补充**
   - wholeNumberOrderedBytes() 方法：补充行号引用（第 78-81 行）

8. **DataFile.java 字段说明优化**
   - lower_bounds 和 upper_bounds 字段说明：明确指出值序列化为 ByteBuffer 格式

**验证结论**: 
- 文档中的所有类名、方法名、字段名均准确无误
- 行号引用已全部修正为与源码完全一致
- 代码逻辑描述与实际实现完全匹配
- 索引机制的技术原理阐述准确，符合 Iceberg 设计哲学
