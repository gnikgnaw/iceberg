...
public interface Snapshot extends Serializable {
  long sequenceNumber();       // 第42行 - 快照的序列号，提交时分配
  long snapshotId();           // 第49行 - 快照的唯一ID
  Long parentId();             // 第56行 - 父快照ID，首个快照为null
  long timestampMillis();      // 第65行 - 快照创建时间戳（毫秒）
  String operation();          // 第97行 - 产生此快照的操作类型
  Map<String, String> summary(); // 第104行 - 操作摘要信息
  String manifestListLocation(); // 第171行 - manifest list 文件位置
  Integer schemaId();          // 第179行 - 创建此快照时使用的schema ID（default方法）
}
```

**BaseSnapshot 实现**（`core/src/main/java/org/apache/iceberg/BaseSnapshot.java`，第36-49行）中明确存储了 `parentId`：

```java
class BaseSnapshot implements Snapshot {
  private final long snapshotId;
  private final Long parentId;           // 第38行 - 父快照ID，形成链表
...
在 `TableMetadata.Builder.setRef()` 方法中（第1319-1329行），可以看到这个区分逻辑：
...
public static Long nullableSnapshotIdAsOfTime(Table table, long timestampMillis) {
  ...
  for (HistoryEntry logEntry : table.history()) {
    if (logEntry.timestampMillis() <= timestampMillis) {
      snapshotId = logEntry.snapshotId();  // 第395行 - 找到不超过给定时间戳的最新快照
    }
  }
...
public class SnapshotRef implements Serializable {
  public static final String MAIN_BRANCH = "main";  // 第28行 - 主分支固定名称

  private final long snapshotId;            // 第30行 - 指向的快照ID
  private final SnapshotRefType type;       // 第31行 - BRANCH 或 TAG
  private final Integer minSnapshotsToKeep; // 第32行 - 分支上最少保留的快照数（仅branch）
  private final Long maxSnapshotAgeMs;      // 第33行 - 分支上快照的最大年龄（仅branch）
  private final Long maxRefAgeMs;           // 第34行 - 引用本身的最大年龄
}
```
...
// 方式2：使用命名引用（branch 或 tag）（第117-129行）
...
**Schema 回溯**：当 `useSnapshotSchema()` 返回 `true` 时（对于 `DataTableScan` 和 `RESTTableScan`），Scan 会自动使用目标快照时的 Schema，而不是当前 Schema。这通过 `SnapshotUtil.schemaFor()` 实现（第412-432行）：
...
public static Schema schemaFor(Table table, long snapshotId) {
  ...
  if (schemaId != null) {
    ...
    Preconditions.checkArgument(
        schema != null, "Cannot find schema with ID %s from table schemas", schemaId);
    return schema;
  }
...
RemoveSnapshots(TableOperations ops) {
  ...
  this.defaultExpireOlderThan = now - defaultMaxSnapshotAgeMs;  // 第96行 - 默认过期时间点
...
**验证范围**: 对照源码验证所有类名、方法名、字段名和行号引用

**修正内容**:

1. **第52-62行** - 修正 `Snapshot` 接口方法的行号引用，补充 `schemaId()` 为 default 方法的说明
2. **第67-78行** - 修正 `BaseSnapshot` 类字段列表，补充完整的字段定义（包括 v1ManifestLocations、firstRowId、addedRows、keyId）
3. **第87-98行** - 修正 `SnapshotLogEntry` 定义的标题（原标题错误写为"HistoryEntry定义"）
4. **第112-124行** - 修正 `TableMetadata.Builder.setRef()` 方法中的注释，使用源码原始英文注释
5. **第127-138行** - 修正 `updateSnapshotLog` 方法中的注释，使用源码原始英文注释
6. **第145-182行** - 修正 `ancestorsOf` 方法代码格式，补充 `consumed` 字段的注释
7. **第199-207行** - 补充 `nullableSnapshotIdAsOfTime` 方法中的行号注释
8. **第239-249行** - 修正 `SnapshotRef` 类字段的行号引用
9. **第628-646行** - 修正 `RemoveSnapshots` 构造函数的代码，使用源码原始错误消息
10. **第601-615行** - 修正 `schemaFor` 方法，补充完整的 Preconditions 检查逻辑
11. **第567-599行** - 修正 `SnapshotScan` 三种时间旅行方式的行号引用和代码注释
12. **第747-765行** - 修正 `cleanExpiredSnapshots` 方法，补充 `Boolean.TRUE.equals(incrementalCleanup)` 的判断逻辑

**验证结论**: 文档整体技术准确性良好，所有类名、方法名、字段名均正确。主要修正了行号引用的精确性、补充了部分遗漏的代码细节、统一使用源码原始注释。核心技术逻辑、实现机制、架构设计的描述完全准确。
