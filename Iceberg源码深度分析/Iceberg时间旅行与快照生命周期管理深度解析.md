...
public interface Snapshot extends Serializable {
  ...
  long snapshotId();           // 第45行 - 快照的唯一ID
  Long parentId();             // 第48行 - 父快照ID，首个快照为null
  long timestampMillis();      // 第51行 - 快照创建时间戳（毫秒）
  String operation();          // 第57行 - 产生此快照的操作类型
  Map<String, String> summary(); // 第63行 - 操作摘要信息
  String manifestListLocation(); // 第171行 - manifest list 文件位置
  Integer schemaId();          // 第179行 - 创建此快照时使用的schema ID
}
```
