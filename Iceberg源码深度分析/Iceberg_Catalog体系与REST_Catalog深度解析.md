...
3. 如果 `initializeCatalogTables=true`，自动创建 Catalog 表和 Namespace 属性表
...
**AuthManagers 加载机制**（`AuthManagers.java` 第39-134行）：

```java
// AuthManagers.java 第88-106行
switch (authType.toLowerCase(Locale.ROOT)) {
  case "none":   impl = AuthProperties.AUTH_MANAGER_IMPL_NONE;   break;
  case "basic":  impl = AuthProperties.AUTH_MANAGER_IMPL_BASIC;  break;
  case "sigv4":  impl = AuthProperties.AUTH_MANAGER_IMPL_SIGV4;  break;
  case "google": impl = AuthProperties.AUTH_MANAGER_IMPL_GOOGLE; break;
  case "oauth2": impl = AuthProperties.AUTH_MANAGER_IMPL_OAUTH2; break;
  default:       impl = authType;  // 支持自定义实现类名
}
```
...
AuthManager
  ...
  └── tableSession()    → 表级别独立会话（表级别 Token）
```

### 7.4 初始化流程

**源码位置**: `RESTSessionCatalog.java` 第195-288行
...
```java
// RESTSessionCatalog.java 第200-217行
...
**源码位置**: `RESTSessionCatalog.java` 第447-559行

```java
// RESTSessionCatalog.java 第447-454行
@Override
...
**创建表**（`RESTSessionCatalog.java` 第954-1000行）：
...
**RESTTableOperations 的 commit 流程**（`RESTTableOperations.java` 第157-220行）：
...
**CommitStateUnknown 容错**（第207-214行）：当遇到 `CommitStateUnknownException` 时，对于纯 snapshot 追加操作，REST Catalog 会尝试轻量级的状态协调——刷新表并检查预期的 snapshot 是否已存在。

### 7.7 多表事务提交

**源码位置**: `RESTSessionCatalog.java` 第1364-1382行
...
**DEFAULT_ENDPOINTS**（`RESTSessionCatalog.java` 第126-143行）：当服务器的 `GET /v1/config` 响应中没有 `endpoints` 字段时，客户端假定服务器支持一组默认端点。这保证了向后兼容性。
...
4. **REST Catalog** 通过双层委托架构 (`RESTCatalog` → `RESTSessionCatalog`)、三级认证模型 (`catalog` → `context` → `table`)、增量变更协议 (`UpdateRequirement` + `MetadataUpdate`) 和渐进式 Endpoint 发现，构建了一个完整的、面向未来的 Catalog 服务标准。
