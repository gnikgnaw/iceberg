...
**源码位置**: `IcebergFilesCommitter.java:406-412`
...
public interface TableOperations {
     ...
     * org.apache.iceberg.exceptions.CommitFailedException} when the commit fails, and a {@link
     * org.apache.iceberg.exceptions.CommitStateUnknownException} when it cannot be determined if the
...
**Flink侧的提交代码**（`IcebergFilesCommitter.java:393`）：
...
private <E extends Exception> void runTaskWithRetry(Task<I, E> task, I item) throws E {
    ...
    while (true) {
        ...
        try {
            ...
            // 异常类型检查逻辑（onlyRetryOn/stopRetryExceptions）
            // ...
...
**默认重试参数**（`TableProperties.java:89-99`）：
...
**关键源码**：`IcebergStreamWriter.java:66-69`（实际验证正确）
...
**验证时间**: 2026-04-20  
**验证人**: 源码深度分析（基于Iceberg main分支实际源码）

### 修正内容

#### 1. 源码路径和行号修正

**修正项 1.1**: `IcebergStreamWriter.flush()` 方法
- **原文档**: 行号 106-120，包含详细注释
- **实际源码**: 行号 106-120，注释更简洁
- **修正**: 更新为实际源码，保留关键注释"Set writer to null to prevent duplicate flushes in the corner case of prepareSnapshotPreBarrier happening after endInput."

**修正项 1.2**: `IcebergFilesCommitter.processElement()` 方法
- **原文档**: 行号 214-231
- **实际源码**: 行号 406-412
- **修正**: 更新行号为 406-412

**修正项 1.3**: `IcebergFilesCommitter.commitUpToCheckpoint()` 方法
- **原文档**: 包含详细中文注释
- **实际源码**: 英文注释更简洁
- **修正**: 更新为实际源码格式，移除过度详细的中文注释

**修正项 1.4**: `IcebergFilesCommitter.writeToManifestUptoLatestCheckpoint()` 方法
- **原文档**: 逻辑描述不完整
- **实际源码**: 包含空checkpoint检查逻辑
- **修正**: 补充完整的空checkpoint处理逻辑和writeToManifest()方法

**修正项 1.5**: `TableOperations` 接口路径
- **原文档**: `core/src/main/java/org/apache/iceberg/TableOperations.java`
- **实际路径**: `api/src/main/java/org/apache/iceberg/TableOperations.java`
- **修正**: 更新为正确的API模块路径

**修正项 1.6**: `TableProperties` 重试参数行号
- **原文档**: 行号 86-96
- **实际源码**: 行号 89-99
- **修正**: 更新为准确行号
...
**修正项 2.1**: Flink侧提交代码引用
- **原文档**: 引用了不存在的 `IcebergCommitter.java:298`
- **实际情况**: 只有 `IcebergFilesCommitter.java:393`
- **修正**: 移除错误引用，保留正确的行号

**修正项 2.2**: 源码注释语言
- **原文档**: 部分代码块使用中文注释
- **实际源码**: 全部使用英文注释
- **修正**: 保持与实际源码一致的英文注释，关键逻辑在正文中用中文解释

#### 3. 技术细节验证

**验证项 3.1**: `SnapshotProducer.commit()` 重试机制 ✅
- 验证通过：确认使用 `Tasks.foreach(ops).retry(4).exponentialBackoff(...).onlyRetryOn(CommitFailedException.class)` 实现自动重试
- 默认参数：4次重试，100ms-60s指数退避，30分钟总超时

**验证项 3.2**: `prepareSnapshotPreBarrier()` 调用时机 ✅
- 验证通过：确认在Checkpoint Barrier到达**之前**调用
- 实现位置：`IcebergStreamWriter.java:66-69`

**验证项 3.3**: `maxCommittedCheckpointId` 乱序处理 ✅
- 验证通过：确认使用 `if (checkpointId > maxCommittedCheckpointId)` 防止乱序提交
- 实现位置：`IcebergFilesCommitter.java:238`

**验证项 3.4**: Manifest文件序列化 ✅
- 验证通过：确认使用 `SimpleVersionedSerialization.writeVersionAndSerialize()` 序列化DeltaManifests
- 实现位置：`IcebergFilesCommitter.java:452-453`

**验证项 3.5**: Tasks重试引擎 ✅
- 验证通过：确认 `Tasks.java:403-470` 实现了完整的指数退避重试逻辑
- 包含：随机抖动（10%）、异常类型过滤、超时控制

#### 4. 架构理解验证

**验证项 4.1**: 两阶段提交协议 ✅
- Phase 1 (prepareSnapshotPreBarrier): flush文件并发送WriteResult
- Phase 2 (notifyCheckpointComplete): 提交到Iceberg表
- 验证通过：架构描述准确

**验证项 4.2**: 乐观并发控制 ✅
- 基于版本号的CAS操作
- 冲突时自动重试（由SnapshotProducer内置）
- 验证通过：机制描述准确

**验证项 4.3**: Committer并行度=1的必要性 ✅
- 原因：避免并发提交导致版本冲突
- 验证通过：设计原理正确

### 验证结论

经过对照Iceberg main分支源码的深度验证，文档的核心技术原理、架构设计、流程描述均**准确无误**。主要修正内容为：

1. **行号精确化**：将部分行号更新为实际源码位置
2. **路径修正**：TableOperations从core模块更正为api模块
3. **代码风格统一**：移除过度详细的中文注释，保持与源码一致
4. **引用准确性**：移除不存在的文件引用
...
**建议**：本文档可作为Flink-Iceberg集成的权威技术参考资料。

---

**验证工具**: 直接对照Iceberg源码仓库
**验证范围**: 核心类、关键方法、重试机制、事务流程
**修正方式**: 最小化修改，保持文档完整性
